...
def edit_TC(request,id,tid):
    if 'email' in request.session and 'id' in request.session:
        ...
        if request.method == 'POST':
            ...
            return render(request,"release_sheet.html",{'tcs':tc,'users':users,'scope':scope,'editobj':testcase})


def filter_TC(request,id):
    if 'email' in request.session and 'id' in request.session:
        scope = Scope.objects.get(id = id)
        title = TestCase_Title.objects.filter(scope_id = scope)
        objs = TestCase.objects.get(title_id = title)
        users = User.objects.all()
        ...
        if request.method == 'POST':
            ...
            for obj in objs:
                if obj.ts_portal == test_status and obj.tester_portal.uname == tester:
                    portal = True

                if obj.ts_rm == test_status and obj.tester_rm.uname == tester:
                    rm = True

                if obj.ts_internal == test_status and obj.tester_internal.uname == tester:
                    internal = True

                if obj.ts_be == test_status and obj.tester_be.uname == tester:
                    be = True

                if obj.ts_ios == test_status and obj.tester_ios.uname == tester:
                    ios = True

                if obj.ts_android == test_status and obj.tester_android.uname == tester:
                    android = True

                if obj.ts_automation == test_status and obj.tester_automation.uname == tester:
                    automation = True
...
def view_TCs(request,id):
    if 'email' in request.session and 'id' in request.session:
        ...
        try:
            ...
            return render(request,"release_sheet.html",{'tcs':obj,'users':users,'scope':scope,'title':title})
    ...
    if 'email' in request.session and 'id' in request.session:
        ...
        if request.method == "POST":
            ...
            try:
                ...
                return render(request,"release_sheet.html",{'tcs':tc,'users':users,'scope':scope,'add_title'})
            return render(request,"release_sheet.html",{'users':users,'scope':scope})
