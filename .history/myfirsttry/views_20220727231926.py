...
def release_scope(request,id):
    if 'email' in request.session and 'id' in request.session:
        release = Release.objects.get(pkk = id)
...
def card_add(request):
    if 'email' in request.session and 'id' in request.session:
        if request.method == "POST":
            ...
            print("-----------------------------------" + str(card_obj.pkk) + " " + str(card_obj.release_num))
            card_obj.save()
...
def scope_add(request,id):
    if 'email' in request.session and 'id' in request.session:
        if request.method == "POST":
            ...
            release_id = Release.objects.get(pkk = id)
...
def del_card(request,id):
    if 'email' in request.session and 'id' in request.session:
        obj = Release.objects.get(pkk = id)
        obj.delete()
        return redirect ('/cards')
    else:
        return redirect('/')
...
def del_scope(request,id):
    if 'email' in request.session and 'id' in request.session:
        ...
        obj.delete()
        return HttpResponseRedirect('/release_scope/' + str(card_id.pkk))
    else:
        return redirect('/')
...
def edit_scope(request,id,rid):
    if 'email' in request.session and 'id' in request.session:
        obj = Scope.objects.get(id = id)
        release = Release.objects.get(pkk = rid)
        if request.method == 'POST':
            epic = request.POST.get('epic')
