...
def show_chart_Tc(request,id):
    if 'email' in request.session and 'id' in request.session:
        ...
        objs = TestCase.objects.filter(title_id = title)
        tcs = TestCase.objects.filter(title_id = title).values('title_id').annotate(
        portal_pass_count=Count('title_id',filter=Q(ts_portal='Pass')),
        portal_fail_count=Count('title_id',filter=Q(ts_portal='Fail')),
        portal_skip_count=Count('title_id',filter=Q(ts_portal='QA Skip')),
        portal_yts_count=Count('title_id',filter=Q(ts_portal='YTS')),
        portal_inprogress_count=Count('title_id',filter=Q(ts_portal='In_progress'))
        )
