...
def login(request):
    if request.method == "POST":
        ...
        try:
            ...
            messages.error(request,"User doesn't exists!! Pls try to login with existing user, or sign up first!!")
            return redirect("/login")
...
def edit_scope(request,id,rid):
    if 'email' in request.session and 'id' in request.session:
        ...
        if request.method == 'POST':
            ...
            all_obj = Scope.objects.filter(release_id = release)
...
def edit_title(request,id):
    if 'email' in request.session and 'id' in request.session:
        print("---------------------Reached in edit title------------------------")
        title = TestCase_Title.objects.get(title_id = id)
        scope = Scope.objects.get(id = title.scope_id.id)
        ...
        if request.method == 'POST':
            ...
            title.title = titlee
            title.tc_author = author
            title.save()

            return redirect("/view_tcs/" + str(scope.id))

        else:
            users = User.objects.all()
            testcase = TestCase.objects.filter(title_id = title)
            return render(request,"release_sheet.html",{'tcs':testcase,'users':users,'scope':scope,'edit_t':title,'title':title})
...
def show_chart_Tc(request,id):
    if 'email' in request.session and 'id' in request.session:
        ...
        for obj in objs:
            ...
            elif obj.ts_be == 'In_progress':
                dic_be['inprogress_count'] += 1
                
            elif obj.ts_be == 'QA Skip':
                dic_be['skip_count'] += 1
        ...
        mainlist.append(dic_rm)
        mainlist.append(dic_internal)
        mainlist.append(dic_android)
        mainlist.append(dic_ios)
        mainlist.append(dic_be)
        mainlist.append(dic_automation)
...
def show_chart_Tc_scope(request,id):
    if 'email' in request.session and 'id' in request.session:
        users = User.objects.all()
        scope = Scope.objects.filter(release_id = id)
        ...
        for s in scope:
            title = TestCase_Title.objects.get(scope_id = s)
        ...
        return render(request,"release_scope.html",{
        'users':users,
        'scopes':scope,
        'release':release,
        'dataset':mainlist})
