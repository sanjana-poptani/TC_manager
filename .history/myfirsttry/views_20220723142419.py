...
def release_scope(request,id):
    if 'email' in request.session and 'id' in request.session:
        ...
        try:
            release_scopes = Scope.objects.get(release_id = id)
            ...
            print("-----------------------I got a release card!!!!---------------" + strrelease.id)
            return render(request,"release_scope.html",{'release' : release.id})
    else:
        return redirect("/")
...
def scope_add(request,id):
    if 'email' in request.session and 'id' in request.session:
        if request.method == "POST":
            ...
            scope_obj = Scope.objects.create(epic = epic,scope_desc = edesc,release_id = release_id,user_id = request.user.id)
            scope_obj.save()

        return redirect('/release_scope/' + id)
    else:
        return redirect('/')
