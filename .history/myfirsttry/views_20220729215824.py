...
def edit_TC(request,id,tid):
    if 'email' in request.session and 'id' in request.session:
        ...
        print(title)
        testcase = TestCase.objects.get(title_id = title)
        ...
        if request.method == 'POST':
            ...
            return render(request,"release_sheet.html",{'tcs':testcase,'users':users,'scope':scope,'editobj':testcase})
